package com.ncs.resturant.core.security;
...
public class SecurityConfiguration {
	...
	/**
	 * Bean for configuring HTTP security with authorization and authentication
	 * rules.
	 * 
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http.authorizeHttpRequests(authorize -> authorize.requestMatchers("/login", "/public/**").permitAll() // Allow
																												// unauthenticated
																												// access
																												// to
																												// login
				...
				.requestMatchers("/user/**").hasAnyRole("USER", "ADMIN") // Allow access to /user/** for USER and ADMIN
																			// roles
				.anyRequest().authenticated() // All other requests require authentication
		).formLogin(form -> form.loginPage("/login") // Custom login page
				.defaultSuccessUrl("/home", true) // Redirect to home after successful login
				.permitAll() // Allow all to access login page
		).logout(logout -> logout.permitAll() // Allow all to log out
		);
